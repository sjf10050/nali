...
- 支持多语言
- 查询完全离线
- 全平台支持（Linux / macOS / Windows / FreeBSD，包括多种 MIPS 架构）
- 支持彩色输出
- 支持 JSON 格式输出
- Shell 自动补全支持
- 内置 LRU 缓存和输出缓冲，高性能
...
Nali 需要预先安装 Go >= 1.23。安装后可以从源码安装软件:
...
使用 `-j` 或 `--json` 参数以 JSON 格式输出，方便与其他工具集成：
...
和 IPv4 用法完全相同。nali 还会自动将 NAT64 地址 (`64:ff9b::/96`) 转换为 IPv4 进行查询。
...
程序第一次运行后，会在 config 目录生成配置文件 `config.yaml` (`nali info` 可查看具体路径)，配置文件定义了数据库信息，默认用户无需进行修改。
...
用户可以指定使用哪个数据库，通过环境变量 `NALI_DB_IP4`、`NALI_DB_IP6`、`NALI_DB_CDN` 或者配置文件 `config.yaml` 中的 `selected` 字段进行设置。
...
通过环境变量 `NALI_LANG` 来指定使用的语言，当使用非中文语言时仅支持GeoIP2这个数据库。
...
设置环境变量 `NALI_HOME` 来指定工作目录，配置文件和数据库存放在同一工作目录下。也可在配置文件中使用绝对路径指定其他数据库路径。

如需分别指定，可设置环境变量 `NALI_CONFIG_HOME` 来指定配置文件目录，`NALI_DB_HOME` 来指定数据库文件目录。
...
nali 支持为 bash、zsh、fish、powershell 等 shell 生成自动补全脚本：
